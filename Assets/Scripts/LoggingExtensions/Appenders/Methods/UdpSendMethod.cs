...
namespace Assets.Scripts.LoggingExtensions.Appenders.Methods
{
	internal class UdpSendMethod : NetworkMethodBase<string>
	{
		...
		private class BufferContainer
		{
			...
			internal int Emitters { get { lock(_emitters) { return _emitters.Count; } } }
			internal bool IsValuable { get { return Buffer != null && Buffer.Length > 0; } }
			internal int Size { get { return Buffer == null ? 0 : Buffer.Length; } }
			...
			: base(cursor, filter)
		{
			_targets =
				connectionString
					...
					.SelectMany(record =>
					{
						...
						var address = tokens.Length > 0 ? tokens[0] : DEFAULT_ADDRESS;
						var portToken = tokens.Length > 1 ? tokens[1] : DEFAULT_PORT;
		    ...
		    foreach(var target in _targets)
			    LogLog.Debug(GetType(), string.Format("start emit from: {0}:{1}", target.Address, target.Port));
		}
